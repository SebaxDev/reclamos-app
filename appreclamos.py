...
df_clientes["N¡ã de Precinto"] = df_clientes["N¡ã de Precinto"].apply(lambda x: str(int(x)).strip() if isinstance(x, (int, float)) else str(x).strip())
df_reclamos["N¡ã de Precinto"] = df_reclamos["N¡ã de Precinto"].apply(lambda x: str(int(x)).strip() if isinstance(x, (int, float)) else str(x).strip())
...
st.markdown("""
    ...
    /* Contenedores de expansiè´¸n */
    .stExpander {
        border-radius: 8px;
        border: 1px solid #dee2e6;
    }
    
    /* Mejoras para mè´¸viles */
...
# --- MEN¨² DE NAVEGACI¨®N ---
opcion = st.radio("?? Ir a la secci¨®n:", ["Inicio", "Reclamos cargados", "Historial por cliente", "Editar cliente", "Imprimir reclamos", "Seguimiento t¨¦cnico", "Cierre de Reclamos"], horizontal=True)

# --- SECCI¨®N 1: INICIO ---
if opcion == "Inicio":
    st.subheader("?? Cargar nuevo reclamo")
    nro_cliente = st.text_input("?? N¡ã de Cliente").strip()
        ...
        with st.form("reclamo_formulario"):
            ...
            if cliente_existente is not None:
                with col1:
                    ...
                    direccion = st.text_input("?? Direcci¨®n", value=cliente_existente["Direcci¨®n"])
                with col2:
            ...
            precinto = st.text_input("?? N¡ã de Precinto (opcional)", value=cliente_existente.get("N¡ã de Precinto", "").strip() if cliente_existente else "")
...
if opcion == "Reclamos cargados":
    ...
    try:
        ...
        df_clientes["No Cliente"] = df_clientes["No Cliente"].astype(str).str.strip()
        df = pd.merge(df, df_clientes[["No Cliente", "N¡ã de Precinto"]], on="No Cliente", how="left", suffixes=("", "_cliente"))
        df["Fecha y hora"] = pd.to_datetime(df["Fecha y hora"], errors="coerce")
        df = df.sort_values("Fecha y hora", ascending=False)

        # --- NUEVO PANEL VISUAL ---
        st.markdown("### ?? Distribuci¨®n por tipo de reclamo (solo activos)")
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "N¡ã de Precinto": st.column_config.TextColumn("N¡ã de Precinto")
            }
        )
        ...
        if st.button("?? Guardar cambios en Google Sheets"):
            try:
                ...
                precinto_dict = edited_df.set_index("No Cliente")["N¡ã de Precinto"].to_dict()
...
# --- SECCI¨®N 3: HISTORIAL POR CLIENTE ---
if opcion == "Historial por cliente":
    st.subheader("?? Historial de reclamos por cliente")
    historial_cliente = st.text_input("?? Ingres¨¢ N¡ã de Cliente para ver su historial").strip()
    ...
    if historial_cliente:
        ...
        if not historial.empty:
            ...
            st.dataframe(
                historial[
                    ["Fecha y hora", "Tipo de reclamo", "Estado", "T¨¦cnico", "N¡ã de Precinto", "Detalles"]
                ],
                use_container_width=True
            )
        else:
...
# --- SECCI¨®N 4: EDITAR CLIENTE ---
if opcion == "Editar cliente":
    st.subheader("??? Editar datos de un cliente")
    cliente_editar = st.text_input("?? Ingres¨¢ N¡ã de Cliente a editar").strip()
    ...
    if cliente_editar:
        ...
        if not cliente_row.empty:
            ...
            nueva_direccion = st.text_input("?? Direcci¨®n", value=cliente_actual["Direcci¨®n"])
            nuevo_telefono = st.text_input("?? Tel¨¦fono", value=cliente_actual["Tel¨¦fono"])
            nuevo_precinto = st.text_input("?? N¡ã de Precinto", value=cliente_actual.get("N¡ã de Precinto", ""))
    ...
    with st.form("form_nuevo_cliente"):
        nuevo_nro = st.text_input("?? N¡ã de Cliente (nuevo)").strip()
        nuevo_sector = st.text_input("??? Sector")
        nuevo_nombre = st.text_input("?? Nombre")
        nueva_direccion = st.text_input("?? Direcci¨®n")
        nuevo_telefono = st.text_input("?? Tel¨¦fono")
        nuevo_precinto = st.text_input("?? N¡ã de Precinto (opcional)")

        guardar_cliente = st.form_submit_button("?? Guardar nuevo cliente")

        if guardar_cliente:
            if not nuevo_nro or not nuevo_nombre:
                st.error("?? Deb¨¦s ingresar al menos el N¡ã de cliente y el nombre.")
...
if opcion == "Imprimir reclamos":
    ...
    try:
        ...
        df_merged = pd.merge(df_pdf, df_clientes[["No Cliente", "N¡ã de Precinto"]], on="No Cliente", how="left")

        st.info("?? Reclamos pendientes de resoluci¨®n")
        ...
        # --- NUEVA FUNCI¨®N: Imprimir por tipo de reclamo ---
            ...
            if not reclamos_filtrados.empty:
                ...
                if st.button("?? Generar PDF de reclamos por tipo"):
                    ...
                    for i, (_, reclamo) in enumerate(reclamos_filtrados.iterrows()):
                        ...
                        precinto_str = f" - Precinto: {reclamo['N¡ã de Precinto']}" if reclamo.get("N¡ã de Precinto") else ""
                        lineas = [
                            f"Fecha: {reclamo['Fecha y hora']} - Cliente: {reclamo['Nombre']} ({reclamo['No Cliente']})",
                            f"Direcci¨®n: {reclamo['Direcci¨®n']} - Tel: {reclamo['Tel¨¦fono']}",
                            f"Sector: {reclamo['Sector']} - Precinto: {reclamo.get('N¡ã de Precinto', '')} - T¨¦cnico: {reclamo['T¨¦cnico']}",
...
# --- SECCI¨®N 6: SEGUIMIENTO T¨¦CNICO ---
import time  # Asegurate de tener esta l¨ªnea al inicio del archivo si no estaba

if opcion == "Seguimiento t¨¦cnico":
    st.subheader("?? Seguimiento t¨¦cnico del reclamo")
    cliente_input = st.text_input("?? Ingres¨¢ el N¡ã de Cliente para actualizar su reclamo").strip()
    ...
    if cliente_input:
        ...
        if df_filtrado.empty:
            ...
            if df_filtrado.empty:
                ...
                st.write(f"?? Direcci¨®n: {reclamo_actual['Direcci¨®n']}")
                st.write(f"?? Precinto: {reclamo_actual.get('N¡ã de Precinto', '')}")
        ...
        for i, row in en_curso.iterrows():
            with st.container():
                ...
                with col1:
                    ...
                    precinto_actual = cliente_info["N¡ã de Precinto"].values[0] if not cliente_info.empty else ""
