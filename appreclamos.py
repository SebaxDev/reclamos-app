...
if opcion == "Editar cliente":
    ...
    if cliente_editar:
        ...
        st.divider()
        st.subheader("🧹 Eliminar reclamos de este cliente")
        df_reclamos["Nº Cliente"] = df_reclamos["Nº Cliente"].astype(str).str.strip()
        reclamos_cliente = df_reclamos[df_reclamos["Nº Cliente"] == cliente_editar]

        if not reclamos_cliente.empty:
            reclamos_cliente = reclamos_cliente.reset_index(drop=True)
            opciones = [f"{i+1}. {row['Fecha y hora']} - {row['Tipo de reclamo']} - {row['Estado']}" for i, row in reclamos_cliente.iterrows()]
            seleccion = st.selectbox("Seleccioná un reclamo para eliminar:", opciones)
            index_local = opciones.index(seleccion)

            with st.expander("⚠️ Confirmar eliminación"):
                st.warning("Esta acción no se puede deshacer. El reclamo será eliminado permanentemente del registro.")
                if st.button("❌ Eliminar reclamo seleccionado"):
                    try:
                        fila_en_hoja = df_reclamos[df_reclamos["Nº Cliente"] == cliente_editar].index[index_local] + 2
                        sheet_reclamos.delete_rows(fila_en_hoja + 1)  # +1 por encabezado
                        st.success("🗑️ Reclamo eliminado exitosamente.")
