...
# --------------------------
# SECCIÓN 8: ESTADÍSTICAS Y EVOLUCIÓN
# --------------------------
    ...
    st.markdown('<div class="section-container">', unsafe_allow_html=True)
    st.subheader("📊 Estadísticas de reclamos")

    # Convertir fechas correctamente
    df_estad = df_reclamos.copy()
    df_estad["Fecha y hora"] = pd.to_datetime(df_estad["Fecha y hora"], errors="coerce")
    df_estad = df_estad.dropna(subset=["Fecha y hora"])
    df_estad["Mes"] = df_estad["Fecha y hora"].dt.strftime("%B %Y")

    # --- VISTA HISTÓRICA GENERAL ---
    st.markdown("### 📌 Histórico General")

    total_reclamos = len(df_estad)
    st.metric("🔢 Total de reclamos", total_reclamos)

    # Reclamos por tipo
    tipo_count = df_estad["Tipo de reclamo"].value_counts().reset_index()
    tipo_count.columns = ["Tipo de reclamo", "Cantidad"]
    fig_tipo = px.bar(tipo_count, x="Tipo de reclamo", y="Cantidad", title="Reclamos por tipo")
    st.plotly_chart(fig_tipo, use_container_width=True)

    # Sectores con más reclamos
    sector_count = df_estad["Sector"].value_counts().nlargest(5).reset_index()
    sector_count.columns = ["Sector", "Cantidad"]
    fig_sector = px.bar(sector_count, x="Sector", y="Cantidad", title="Top 5 sectores con más reclamos")
    st.plotly_chart(fig_sector, use_container_width=True)

    # Técnicos con más resueltos
    tecnicos = df_estad[df_estad["Estado"] == "Resuelto"]
    tecnicos_expandido = tecnicos["Técnico"].fillna("").str.split(",").explode().str.strip()
    tecnico_count = tecnicos_expandido.value_counts().reset_index()
    tecnico_count.columns = ["Técnico", "Cantidad"]
    fig_tecnico = px.bar(tecnico_count, x="Técnico", y="Cantidad", title="Reclamos resueltos por técnico")
    st.plotly_chart(fig_tecnico, use_container_width=True)

    st.divider()

    # --- FILTRO: HISTÓRICO / MENSUAL / SEMANAL ---
    st.markdown("### 📅 Filtros de tiempo")
    modo = st.selectbox("Elegí vista temporal:", ["Histórico", "Mensual", "Semanal"])

    df_filtrado = df_estad.copy()
    titulo_pdf = "Informe de reclamos"

    if modo == "Mensual":
        meses_disponibles = df_filtrado["Mes"].sort_values(ascending=False).unique()
        mes_seleccionado = st.selectbox("Seleccioná un mes", meses_disponibles)
        df_filtrado = df_filtrado[df_filtrado["Mes"] == mes_seleccionado]
        titulo_pdf += f" - {mes_seleccionado}"

    elif modo == "Semanal":
        hoy = datetime.today()
        hace_una_semana = hoy - timedelta(days=6)
        df_filtrado = df_filtrado[df_filtrado["Fecha y hora"] >= hace_una_semana]
        titulo_pdf += f" - Semana actual ({hace_una_semana.strftime('%d/%m')} al {hoy.strftime('%d/%m')})"

    if modo != "Histórico":
        st.markdown(f"### 📊 Datos filtrados por: **{modo}**")

        st.metric("📌 Reclamos en el período", len(df_filtrado))

        # Por tipo
        tipo_f = df_filtrado["Tipo de reclamo"].value_counts().reset_index()
        tipo_f.columns = ["Tipo de reclamo", "Cantidad"]
        st.plotly_chart(px.bar(tipo_f, x="Tipo de reclamo", y="Cantidad", title="Tipos de reclamo"), use_container_width=True)

        # Por sector
        sector_f = df_filtrado["Sector"].value_counts().nlargest(5).reset_index()
        sector_f.columns = ["Sector", "Cantidad"]
        st.plotly_chart(px.bar(sector_f, x="Sector", y="Cantidad", title="Top 5 sectores"), use_container_width=True)

        # Por técnico resuelto
        tecnicos_f = df_filtrado[df_filtrado["Estado"] == "Resuelto"]
        tecnicos_exp = tecnicos_f["Técnico"].fillna("").str.split(",").explode().str.strip()
        tecnico_f = tecnicos_exp.value_counts().reset_index()
        tecnico_f.columns = ["Técnico", "Cantidad"]
        st.plotly_chart(px.bar(tecnico_f, x="Técnico", y="Cantidad", title="Resueltos por técnico"), use_container_width=True)
        ...
        if st.button("📄 Exportar informe a PDF"):
            ...
            c.drawString(40, y, f"Total de reclamos: {len(df_filtrado)}")
            y -= 20

            for label, df_data in [
                ("Tipos de reclamo", tipo_f),
                ("Top sectores", sector_f),
                ("Resueltos por técnico", tecnico_f)
            ]:
            ...
            st.download_button(
                ...
                data=buffer,
                file_name="informe_reclamos.pdf",
                mime="application/pdf"
            )
    ...
    evolucion_tipo = df_estad.copy()
    evolucion_tipo["Mes corto"] = evolucion_tipo["Fecha y hora"].dt.to_period("M").astype(str)

    tipo_estado = st.radio("Ver evolución por:", ["Estado", "Tipo de reclamo"], horizontal=True)

    fig_evol = px.bar(
        evolucion_tipo.groupby(["Mes corto", tipo_estado]).size().reset_index(name="Cantidad"),
        x="Mes corto", y="Cantidad", color=tipo_estado,
        barmode="stack", title=f"Evolución mensual por {tipo_estado.lower()}"
    )
